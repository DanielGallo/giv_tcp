...
for inv in range(1,int(os.getenv('NUMINVERTERS'))+1):
    logger.critical ("Setting up inverter: "+str(inv)+" of "+str(os.getenv('NUMINVERTERS')))
    ...
    if exists(os.getenv("CACHELOCATION")+"/regCache_"+str(inv)+".pkl"):
        logger.critical("Removing old inverter data cache")
...
########### Run the various processes needed #############
    ...
    if os.getenv('SELF_RUN')=="True":
        logger.critical ("Running Inverter read loop every "+str(os.getenv('SELF_RUN_LOOP_TIMER'))+"s")
